...
resource "aws_ecs_task_definition" "ecs-service-taskdef" {
  family                = "${var.APPLICATION_NAME}" # A family groups together revisions of a task definition. When you make changes to a task definition, ECS creates a new revision within the same family
...
resource "aws_ecs_service" "ecs-service" {
  name                               = "${var.APPLICATION_NAME}" # Name of the ECS service
  cluster                            = "${var.CLUSTER_ARN}" # Associates this service with a specific ECS cluster
  ...
  iam_role                           = "${var.SERVICE_ROLE_ARN}" # IAM role that grants the ECS service permissions to call other AWS services on your behalf (e.g., registering tasks with the load balancer, interacting with CloudWatch logs)
  desired_count                      = "${var.DESIRED_COUNT}" # The number of tasks that you want to run for this service. ECS will automatically maintain this number
  ...
  load_balancer { # Integrates the ECS service with an ALB
    ...
    container_name   = "${var.APPLICATION_NAME}" # The name of the container within your task definition that exposes the port to the load balancer. Important: The name property inside your `ecs-service.json.tpl` container definition must match this name
    container_port   = "${var.APPLICATION_PORT}" # The port on the specified container that the load balancer should forward traffic to. Important: This port must be exposed in your `ecs-service.json.tpl` container definition's portMappings
  }
